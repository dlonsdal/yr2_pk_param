...
* %GCV is the **Geometric Coefficient of Variation** (expressed as a percentage).
...
data <- tribble(
  ...
  "Arithmetic", "Arithmetic Mean ($\\bar{x}$ or $\\text{AM}$)", "%CV", "$\\left( \\frac{s}{\\bar{x}} \\right) \\times 100$", "$\\text{Normal}$ (Symmetrical) Data",
  "Geometric", "Geometric Mean ($\\text{GM}$)", "%GCV", "$\\sqrt{\\exp(s_{\\ln}^2) - 1} \\times 100$", "$\\text{Log-Normal}$ (Skewed) Data"
)
...
```{r log-pk,eval=FALSE,echo=FALSE}
...
```{r log-pknca,eval=FALSE,echo=FALSE}
...
```{r alternative calculation, echo=FALSE, eval=FALSE}
library(dplyr)
...
I will give you the answer for AUC. This is quite challenging. 
