...
You _will_ need to run the following code in order to utilise the correct pacakges.
...
Rate is quantified usually by comparing $T_{max}$ and $C_{max}$ and extent by AUC. For example 
$$Bioequivalence (F) = 100 .  \frac{AUC_{Test}}{AUC_{Ref}}$$

Where

- $Test$ is the test (new) drug product
- $Ref$ is the established drug product
